...
describe('StudentPostgresImportExportAdapter',()=>{
  it.each(['csv','xlsx','json','xml'] as const)('keeps template/export schema and parses %s previews',async(format)=>{
    ...
    const service=new StudentPostgresImportExportAdapter(pool as never,{run:async<T>(work:(client:never)=>Promise<T>)=>work({} as never)} as never,codecs as never,passwords() as never);
  ...
  it('validates duplicates, legacy gender/date, and missing password without hashing or writes',async()=>{
    ...
    const service=new StudentPostgresImportExportAdapter(pool as never,{run:async<T>(work:(client:never)=>Promise<T>)=>work({} as never)} as never,makeCodecs() as never,hasher as never);
  ...
  it('updates class/hobbies while preserving an existing hash for a blank password',async()=>{
    ...
    const hasher=passwords();
    const service=new StudentPostgresImportExportAdapter({} as never,{run:async<T>(work:(value:typeof client)=>Promise<T>)=>work(client)} as never,makeCodecs() as never,hasher as never);
  ...
  it('rolls back an invalid create before any insert/update',async()=>{
    ...
    const service=new StudentPostgresImportExportAdapter({} as never,{run} as never,makeCodecs() as never,passwords() as never);
