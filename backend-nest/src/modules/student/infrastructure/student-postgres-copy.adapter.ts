...
@Injectable()
export class StudentPostgresCopyAdapter implements StudentCopyPort {
  private readonly logger = new Logger(StudentPostgresCopyAdapter.name);
  ...
  async copyMany(ids: unknown[]) {
    return this.transactions.run(async (client) => {
      const created: StudentPersistenceRecord[] = [];
