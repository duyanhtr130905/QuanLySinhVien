...
export class ClassCommandService {
  ...
  async delete(id: number): Promise<{ id: string }> { const exists = await this.repository.existsById(id); if (!exists) throw classException.deleteNotFound(); try { const deleted = await this.repository.deleteById(id); return { id: deleted?.id ?? String(id) }; } catch (error) { if (error instanceof ForeignKeyViolationError) throw classException.deleteBlocked(error); throw error; } }
